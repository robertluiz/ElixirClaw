defmodule ElixirClaw.Providers.OpenRouter do
  @moduledoc """
  OpenRouter provider implementation for OpenAI-compatible chat completions.
  """
  ...
  defp request_options(messages, opts) do
    with {:ok, api_key} <- fetch_required_config(:api_key, :missing_api_key),
         {:ok, referer_url} <- fetch_required_config(:referer_url, :missing_referer_url),
         {:ok, app_title} <- fetch_required_config(:app_title, :missing_app_title) do
      {:ok,
       [
         url: chat_completions_url(),
         auth: {:bearer, api_key},
         headers: request_headers(referer_url, app_title),
  ...
  defp request_headers(referer_url, app_title) do
    [
